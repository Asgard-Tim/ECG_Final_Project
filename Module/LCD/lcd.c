...
void drawCurve(float value, uint16_t virticalOffset, uint16_t heigt, uint16_t width)
{
    uint16_t x, y;
    value *= 30; // -2-3
    y = virticalOffset + value;
    ...
    {
        lcd_draw_point(10, y, BLACK); // 水平坐标不变，竖直坐标反转
        lastX      = 10;
        lastY      = y;
        firstPoint = 0;
    } else {
        x = lastX + 1;          // 水平坐标正常递增
        if (x < width - 10) // 不超过屏幕宽度
        {
            lcd_draw_line(lastX, lastY, x, y, BLACK); // 绘制线段
            lastX = x;
            lastY = y;
        } else // 超出屏幕宽度，清屏，从第一个点开始绘制
        {
            lcd_clear(WHITE);
            // lcd_fill(10, 10, width - 10, 150, WHITE);
            lcd_draw_point(10, y, BLACK);
            lastX = 10;
            lastY = y;
        }
    }
